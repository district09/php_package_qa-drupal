...
/**
 * @file
 * Bootstrap for PHPUnit tests run within an extension.
 *
 ...
 *   The root directory.
 */
function drupal_phpunit_root_dir()
{
 ...
 */
function drupal_phpunit_find_extension_directories($dirs)
{
    $extensions = [];
    ...
    foreach ($dirs as $extension => $dir) {
        ...
        foreach ($suites as $suite) {
            ...
            if (is_dir($suite_dir)) {
                ...
                $namespaces['Drupal\\Tests\\' . $extension . '\\' . $suite . '\\'][] = $suite_dir;
            }
        }

        // Extensions can have a \Drupal\extension\Traits namespace for
        // cross-suite trait code.
        $traits_dir = $tests_dir . '/Traits';
        if (is_dir($traits_dir)) {
            $namespaces['Drupal\\Tests\\' . $extension . '\\Traits\\'][] = $traits_dir;
        }
    }
...
function drupal_phpunit_populate_class_loader()
{
    ...
    if (!isset($GLOBALS['namespaces'])) {
        ...
        $roots = drupal_phpunit_contrib_extension_directory_roots();
